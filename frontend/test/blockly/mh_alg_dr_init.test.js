...
import { definition } from '../../src/components/blockly/mh_alg_dr_init.js';
...
describe('mh_alg_dr_init block definition', () => {
    it('has category "Algorithms"', () => {
        expect(definition.category).toBe('Algorithms');
    });

    it('has type "mh_alg_dr_init"', () => {
        expect(definition.blockdefinition.type).toBe('mh_alg_dr_init');
    });
...
describe('mh_alg_dr_init generator', () => {
    it('generates correct code', () => {
        const block = makeBlock();
        const gen = makeGenerator();
        const [code, prec] = definition.generator(block, gen);
        expect(code).toBe('alg.initDR()');
        expect(prec).toBe(0);
    });
