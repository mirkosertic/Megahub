...
public:
    ...
    int   getDataAsInt()   const {
        if (formatType_ == FormatType::DATAFLOAT) return (int)floatValue_;
        return intValue_;
    }
    float getDataAsFloat() const {
        if (formatType_ == FormatType::DATAFLOAT) return floatValue_;
...
// ---------------------------------------------------------------------------
static void le16(uint8_t *buf, int16_t v)  { std::memcpy(buf, &v, 2); }
static void le32(uint8_t *buf, int32_t v)  { std::memcpy(buf, &v, 4); }
static void lef32(uint8_t *buf, float v)   { std::memcpy(buf, &v, 4); }
