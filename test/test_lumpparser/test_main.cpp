...
class Mode {
  public:
	...
	Mode()
	    : name_(""), units_(""), pctMin_(0), pctMax_(0), siMin_(0), siMax_(0), format_(nullptr),
	      processDataPacketCalls(0), lastPayloadSize(0) {
...
struct MockLegoDevice {
	...
	MockLegoDevice() { memset(lastDataPayload, 0, sizeof(lastDataPayload)); }
	...
	void onDataFrameDispatched() { onDataFrameDispatchedCalls++; }
...
class LumpParserT {
  public:
	...
	explicit LumpParserT(MockLegoDevice* device)
	    : head_(0), count_(0), extModeOffset_(0), inSyncLoss_(false), consecutiveErrors_(0), device_(device) {
  ...
  private:
	...
	void dispatchFrame(uint8_t header, const uint8_t* payload, int payloadSize) {
		...
		if (type == LP_MSG_TYPE_CMD) {
			...
			switch (cmd) {
				case LP_CMD_TYPE: {
					...
					switch (id) {
						case 37:
							name = "BOOST Color and Distance Sensor";
							break;
						case 38:
				...
				case LP_CMD_SPEED: {
					if (payloadSize >= 4) {
						long s = ((long) payload[0]) | ((long) payload[1] << 8) | ((long) payload[2] << 16) |
						         ((long) payload[3] << 24);
				...
				case LP_CMD_VERSION: {
					if (payloadSize >= 8) {
						char tmp[3];
						std::string fw, hw;
						bcdByteToStr_t(payload[3], tmp);
						fw += tmp;
						fw += '.';
						bcdByteToStr_t(payload[2], tmp);
						fw += tmp;
						fw += '.';
						bcdByteToStr_t(payload[1], tmp);
						fw += tmp;
						fw += '.';
						bcdByteToStr_t(payload[0], tmp);
						fw += tmp;
						bcdByteToStr_t(payload[7], tmp);
						hw += tmp;
						hw += '.';
						bcdByteToStr_t(payload[6], tmp);
						hw += tmp;
						hw += '.';
						bcdByteToStr_t(payload[5], tmp);
						hw += tmp;
						hw += '.';
						bcdByteToStr_t(payload[4], tmp);
						hw += tmp;
		...
		if (type == LP_MSG_TYPE_INFO) {
			...
			if (infoByte & LP_INFO_MODE_PLUS_8) {
				mode += 8;
			}
			...
			switch (infoType) {
				...
				case LP_INFO_MAPPING: {
					...
					if (in & 128) {
						m->registerInputType(Mode::InputOutputType::SUPPORTS_NULL);
					}
					if (in & 64) {
						m->registerInputType(Mode::InputOutputType::SUPPORTS_FUNCTIONAL_MAPPING_20);
					}
					if (in & 16) {
						m->registerInputType(Mode::InputOutputType::ABS);
					}
					if (in & 8) {
						m->registerInputType(Mode::InputOutputType::REL);
					}
					if (in & 4) {
						m->registerInputType(Mode::InputOutputType::DIS);
					}
					if (out & 128) {
						m->registerOutputType(Mode::InputOutputType::SUPPORTS_NULL);
					}
					if (out & 64) {
						m->registerOutputType(Mode::InputOutputType::SUPPORTS_FUNCTIONAL_MAPPING_20);
					}
					if (out & 16) {
						m->registerOutputType(Mode::InputOutputType::ABS);
					}
					if (out & 8) {
						m->registerOutputType(Mode::InputOutputType::REL);
					}
					if (out & 4) {
						m->registerOutputType(Mode::InputOutputType::DIS);
					}
					break;
				}
				case LP_INFO_FORMAT: {
					...
					Format* f = new Format(payload[1], Format::forId(payload[2]), payload[3], payload[4]);
					m->setFormat(f);
					break;
				}
				default:
...
void test_LP13_ext_mode_sets_offset_for_data_frame() {
	...
	uint8_t frame[] = {extHeader, extP[0], extCs, dataHeader, dataP[0], dataCs};
...
void test_LP14_ext_mode_offset_reset_after_data() {
	...
	uint8_t frame[] = {extHeader, extP[0], extCs, data2Header, data2P[0], data2Cs, data1Header, data1P[0], data1Cs};
