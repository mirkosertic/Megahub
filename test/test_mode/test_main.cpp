...
public:
    enum class FormatType {
        DATA8 = 0x00, DATA16 = 0x01, DATA32 = 0x02, DATAFLOAT = 0x03, UNKNOWN = 0xff
    };
    ...
    Format(int ds, FormatType ft, int fig, int dec)
        : datasets_(ds), formatType_(ft), figures_(fig), decimals_(dec) {}
...
public:
    ...
    Mode()
        : pctMin_(0.0f), pctMax_(100.0f), siMin_(0.0f), siMax_(1023.0f) {
        name_ = "";
        units_ = "";
        for (int i = 0; i < 5; i++) {
            inputTypes_[i]  = false;
    ...
    void processDataPacket(const uint8_t *payload, int payloadSize) {
        ...
        switch (format_->getFormatType()) {
            case Format::FormatType::DATA8:     datasetSize = 1; break;
            case Format::FormatType::DATA16:    datasetSize = 2; break;
            case Format::FormatType::DATA32:    datasetSize = 4; break;
            case Format::FormatType::DATAFLOAT: datasetSize = 4; break;
            default:
