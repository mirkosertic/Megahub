...
// ---------------------------------------------------------------------------
static AssemblyResult handleFragment(const uint8_t* data, size_t length,
                                     std::map<uint8_t, FragmentBuffer>& fragmentBuffers,
                                     std::vector<uint8_t>& completedData, uint8_t& completedId) {
	...
	if (protocolType == ProtocolMessageType::STREAM_START || protocolType == ProtocolMessageType::STREAM_DATA ||
	    protocolType == ProtocolMessageType::STREAM_END) {
		return AssemblyResult::STREAM_ROUTED;
	}

	// Memory safety: limit concurrent messages
	if (fragmentBuffers.size() >= MAX_CONCURRENT_MESSAGES && fragmentBuffers.find(messageId) == fragmentBuffers.end()) {
...
//   [5..] = payload bytes
// ---------------------------------------------------------------------------
static std::vector<uint8_t> makeFragment(ProtocolMessageType type, uint8_t messageId, uint16_t fragmentNum,
...
static void test_BT05_max_concurrent_messages_limit() {
	...
	TEST_ASSERT_EQUAL_INT(static_cast<int>(MAX_CONCURRENT_MESSAGES), static_cast<int>(buffers.size()));
...
static void test_BT06_stream_messages_bypass_reassembly() {
	...
	for (auto type :
	     {ProtocolMessageType::STREAM_START, ProtocolMessageType::STREAM_DATA, ProtocolMessageType::STREAM_END}) {
		auto pkt = makeFragment(type, 0x01, 0, FLAG_LAST_FRAGMENT, {0x01, 0x02});
		AssemblyResult r = handleFragment(pkt.data(), pkt.size(), buffers, completed, completedId);
		TEST_ASSERT_EQUAL_INT(static_cast<int>(AssemblyResult::STREAM_ROUTED), static_cast<int>(r));
	}
	TEST_ASSERT_EQUAL_INT(0, static_cast<int>(buffers.size()));
}
